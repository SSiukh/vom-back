...
async function main() {
    ...
    const seenNpStatusCodes = new Set();
    for (const status of shipmentStatuses) {
        for (const npStatusCode of status.npStatusCodes) {
            if (seenNpStatusCodes.has(npStatusCode)) {
                throw new Error(`Nova Poshta status code "${npStatusCode}" is assigned to more than one shipment_statuses row`);
            }
            seenNpStatusCodes.add(npStatusCode);
        }
    }
    await Promise.all(shipmentStatuses.map((status) => prisma.shipmentStatus.upsert({
